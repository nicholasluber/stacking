...
def calculate_deficiency(inpst, inphi, inphierr):
    """
    Calculate HI deficiency using the Maddox+ sample as a the expected mass.
    """
    
    # Define the data from the Maddox sample.
    maddata = np.array([\
    [6.8, 7.73, 0.51],\
    [7.2, 8.12, 0.55],\
    [7.6, 8.35, 0.44],\
    ...
    # Find the appropriate Maddox HI masses.
    theory_hi_masses = np.zeros(len(inphi))
    theory_hi_masses_err = np.zeros(len(inphi))
    for jj in range(0, len(inpst)):
        for ii in range(0, len(maddata)):
            if maddata[ii][0] > inpst[jj]:
                break
        theory_hi_masses[jj] = np.mean(maddata[ii-1:ii+1,1])
        theory_hi_masses_err[jj] = np.mean(maddata[ii-1:ii+1,2])
        
    # Calculate the deficiency.
    defHI = theory_hi_masses - inphi
    defHIerr = np.sqrt(np.square(theory_hi_masses_err)+np.square(inphierr))
    return np.array([defHI,defHIerr])


### Functions for the stacking. ###
...
def HImass_calc(datafile, box, major, minor, pixdim, chan_width):
    ...
    # Get the sum.
    pixsum = np.sum(data[box[2][0]:box[2][1]+1,box[0][0]:box[0][1]+1,box[1][0]:box[1][1]+1])
